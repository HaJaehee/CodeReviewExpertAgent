...
### 출력 토큰 상한은 청크당 지적 수에 맞춥니다

`max_output_tokens` 는 모델이 지적을 쓸 수 있는 분량입니다. 여기에 걸리면 서버는
JSON 문법과 무관하게 그 자리에서 생성을 끊습니다 — 지적 세 건을 쓰고 네 번째의
`suggestion` 에 소스코드를 인용하던 중이었다면, 응답은 따옴표가 열린 채로
끝납니다. 청크마다 코드 길이가 다르니 **어떤 청크는 되고 어떤 청크는 안 되는**
모양으로 나타납니다.

CREX 는 이럴 때 온전히 끝난 지적까지만 살려내고, 잘렸다는 사실을 오류 목록에
적습니다.

```
경고: 오류 1건으로 리뷰가 온전히 끝나지 않았습니다 (생성 1, 검증 0).
  - src/buffer.cpp#1 응답이 잘려 지적 3건만 복구했습니다 — llm.generator.max_output_tokens 를 올리거나 review.max_findings_per_chunk 를 줄이십시오
```

이 경고가 보이면 값을 올리세요. 필요한 양은 `review.max_findings_per_chunk` 에
비례합니다. 지적 하나가 `message` 400자 + `suggestion` 400자까지 쓸 수 있으므로
대략 270토큰이고, 기본값인 5건이면 1400토큰 안팎이 필요합니다. 1600을 권합니다.

반대로 값을 키워도 리뷰가 길어지지는 않습니다. 모델은 할 말이 끝나면 멈춥니다 —
상한은 상한일 뿐입니다.
...
CREX 는 사전 컴포넌트별 토큰 예산제(동적 예산제)를 적용하여 시스템 지시, 룰 택소노미(~1,500토큰),
정적분석 결과(~400토큰), Tree-sitter AST 구문(~400토큰), 소스코드 청크(~4,500토큰)가
8,192 입력 토큰 내에 100% 온전히 수용되도록 보장합니다. 소스코드가 중간에서 잘려 나가지 않습니다.
...
는 400을 넘지 않도록 잘립니다 — 더 크게 적어도 400이 되고, 더 작게 적으면 그
값이 그대로 쓰입니다. 400 아래로 내리는 것은 권하지 않습니다. 검증 근거가
500자까지 허용되므로, 더 조이면 근거가 문장 중간에서 잘립니다 (판정 자체는
`verdict` 가 맨 앞에서 나오므로 살아남습니다).
...
**대개 적을 필요가 없습니다.** 비워두면 바뀐 파일이 속한 `.csproj` 를 찾아 그것만
빌드하고, 여러 프로젝트에 걸쳐 있으면 `.sln` 으로 올립니다. 적어야 하는 경우는
둘입니다 — 저장소에 솔루션이 여러 개라 자동으로 못 정할 때, 또는 일부러 다른
프로젝트를 태우고 싶을 때. 지금 무엇이 잡히는지는 `python -m crex doctor` 의
"빌드 대상" 줄에 나옵니다. 자세한 규칙은
[정적분석 도구 문서](analyzers.md#대상은-알아서-정합니다)에 있습니다.
...
**`absolute_max_lines`** — 배수와 무관한 청크 라인 수 절대 상한입니다 (기본값 **150줄**).
거대 함수나 연속된 여러 hunk 가 병합될 때 이 상한을 넘으면 자동으로 안전하게 독립 청크로
분할하여, LLM 의 입력 토큰 예산(8,192 토큰) 내에 소스코드가 100% 온전히 유지되도록 보장합니다.
`scan` 모드에서는 이 값이 윈도우 크기가 됩니다.

- **토큰 예산과 자동 연동 (Auto-Clamping)**: 만약 `llm.generator.max_input_tokens` 를 4,096 등으로 작게 설정하고 이 값을 크게 두더라도, 파이프라인이 입력 토큰 한계에 맞춰 안전한 라인 수(예: ~50줄)로 **청크 상한을 자동 하향 조절**하므로 설정 불일치로 코드가 잘리는 일이 없습니다.
- **점진적 압축 방어 (Progressive Pruning)**: 한 줄에 수백 자가 넘는 긴 코드가 밀집되어 토큰이 예산을 위협하는 경우, 소스코드는 건드리지 않고 정적분석 지적 수와 룰 목록을 먼저 압축하여 안전하게 수용합니다.
