...
정적분석 도구(clang-tidy, cppcheck, ruff 등)는 **없어도 리뷰가 됩니다.** 있으면
지적의 근거가 늘어 오탐이 줄어듭니다. 설치는 나중에 해도 되니, 일단 여기서는
`doctor` 로 무엇이 빠졌는지만 확인하고 넘어가세요 —
[정적분석 도구](analyzers.md)에 내려받는 곳과 라이선스가 정리되어 있습니다.
...
가능하면 채우세요 — 어디서 받고 라이선스가 무엇인지는
[정적분석 도구](analyzers.md)에 있습니다. LLM 엔드포인트가 실패하면 그건 진짜 막힌 겁니다 —
...
|---|---|
| 0 | 정상 종료. `high` 지적 없음 |
| 1 | `high` 심각도 지적이 하나 이상 |
| 2 | 설정·인자 오류 (git 저장소가 아님, 설정 키 오타 등) |
| 3 | **리뷰가 끝까지 돌지 못함** — 결과를 믿으면 안 됩니다 |

커밋 훅이나 스크립트에서 게이트로 쓸 수 있습니다.

```bash
python -m crex review --staged --out reports/ || echo "심각한 지적이 있습니다"
```

3을 0과 구분하는 이유가 있습니다. LLM 호출이 전부 실패하면 지적이 0건으로
나오는데, 이건 "코드가 깨끗하다"와 겉모습이 같습니다. 그대로 0을 내보내면 고장난
파이프라인이 매번 통과 도장을 찍게 됩니다. CI 에서는 3을 실패로 다루세요.
...
case $? in
  0) echo "통과" ;;
  1) echo "심각한 지적이 있습니다" ;;
  3) echo "리뷰 실패 — python -m crex doctor 를 확인하세요"; exit 1 ;;
esac
```
